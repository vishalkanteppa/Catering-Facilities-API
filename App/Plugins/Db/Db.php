...
class Db implements IDb
{
     ...
     * @return bool
     */
    public function executeQuery(string $query, array $bind = []): PDOStatement|false
    {
        ...
        try {
            if ($bind) {
                $executed = $this->stmt->execute($bind);
            } else {
                $executed = $this->stmt->execute();
            }
        } catch (PDOException $e) {
            error_log("Database error: " . $e->getMessage() . " Query: " . $query);
            return false; // Crucial: Return FALSE on exception
        }

        if ($executed) {
            return $this->stmt;
        } else {
            error_log("executeQuery failed, but no exception was thrown. Query: " . $query);
            return false; // Return FALSE if execute() returned false for some reason
        }
    }
